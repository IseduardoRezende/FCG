...
public interface IUserGameService
{
    ...
    Task<Result<IReadOnlyList<ReadUserGameDto>>> GetLibraryAsync(long? userId, CancellationToken cancellationToken = default);
...
public class UserGameService : IUserGameService
{
    ...
    public async Task<Result<IReadOnlyList<ReadUserGameDto>>> GetLibraryAsync(long? userId, CancellationToken cancellationToken = default)
    {
        var targetUserId = ResolveLibraryUserId(userId);
        if (targetUserId is null)
        {
            return InvalidResult<IReadOnlyList<ReadUserGameDto>>.Create(new Error("User id is required."));
        }

        if (userId.HasValue && userId.Value != _currentUser.UserId && !_currentUser.IsAdministrator)
        {
            return InvalidResult<IReadOnlyList<ReadUserGameDto>>.Create(new Error("You can only access your own library."));
        }

        var items = await _userGameRepository.GetByUserIdAsync(targetUserId.Value, cancellationToken);
        var result = items.Select(MapToReadDto).ToList();
        return SuccessResult<IReadOnlyList<ReadUserGameDto>>.Create(result);
    }
